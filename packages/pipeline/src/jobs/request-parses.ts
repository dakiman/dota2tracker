 ...
 * parsed matches). Only the last 30 days are eligible — replays typically
 * expire after ~2 weeks, but stragglers survive longer and failed requests
 * are cheap and logged. Capped per run to stay polite on the free tier.
 */
...
export async function run(): Promise<string> {
  const rows = await db
    ...
    .from(playerMatches)
    .where(
      sql`${playerMatches.laneRole} IS NULL AND ${playerMatches.startTime} > now() - interval '30 days'`
    )
